...
describe('CheckoutComponent', () => {
  ...
  describe('Form Initialization', () => {
    ...
    it('should have required fields validation', () => {
      ...
      expect(component.checkoutForm.get('orderNotes')?.errors?.['required']).toBeFalsy();
    });

    it('should validate order notes length', () => {
      const control = component.checkoutForm.get('orderNotes');
      const longText = 'a'.repeat(501);

      control?.setValue(longText);
      expect(control?.errors?.['maxlength']).toBeTruthy();

      control?.setValue('Valid note');
